...
func TestWireV1TypedNodeKinds(t *testing.T) {
	...
	if node, _ := explicit.Node("module:package.json#app"); node == nil || node.Kind() != model.NodeKindModule {
		t.Fatalf("module node = %#v", node)
	}
	if dep, ok := explicit.DependencyNode("pkg:npm/left-pad@1.3.0"); !ok || dep.Kind() != model.NodeKindDependency {
		t.Fatal("dependency node lost")
	}

	var legacy model.Graph
	...
	legacy.WalkNodes(func(node model.GraphNode) bool {
		kinds[node.Kind()]++
		return true
	})
	if kinds[model.NodeKindManifest] != 1 || kinds[model.NodeKindModule] != 1 || kinds[model.NodeKindDependency] != 2 {
	...
	if err != nil || len(parents) != 1 || parents[0].Kind() != model.NodeKindModule {
		t.Fatalf("edge did not follow the re-identified node: %v, %v", parents, err)
	}

	var conflicting model.Graph
	...
	if len(nodes) != 1 || nodes[0].Kind() != model.NodeKindModule {
		t.Fatalf("explicit kind must win over the legacy type: %#v", nodes)
	}

	var unknown model.Graph
...
func TestWireV1DuplicateIdentityFolds(t *testing.T) {
	...
	if !dep.HasScope(model.ScopeRuntime) || !dep.HasScope(model.ScopeDevelopment) {
...
func TestWireV1NodeFieldsAreOmitEmpty(t *testing.T) {
	...
	module, err := model.NewModuleNode("package.json", model.Coordinates{Name: "app"})
	if err != nil {
		t.Fatal(err)
	}
	dependency, err := model.NewDependencyNodeFromPURL("pkg:npm/left-pad@1.3.0")
	if err != nil {
		t.Fatal(err)
	}
	...
	forbidden := map[model.NodeKind][]string{
		model.NodeKindManifest:   append([]string{"manifest_kind", "declaring_manifest_path", "origins", "origin", "purl", "scopes", "package_ref"}, assertions...),
		model.NodeKindModule:     append([]string{"manifest_kind", "origins", "origin", "scopes", "package_ref"}, assertions...),
		model.NodeKindDependency: append([]string{"manifest_kind", "declaring_manifest_path", "origins", "origin", "scopes", "package_ref"}, assertions...),
	}

	graph := model.New()
	for _, node := range []model.GraphNode{manifest, module, dependency} {
...
func TestWireV1NestedAssertionFieldsAreOmitEmpty(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name:      "PackageLicense",
			value:     model.PackageLicense{Value: "MIT"},
			forbidden: []string{"spdx_expression", "type", "name", "extracted_text", "source"},
		},
		{
			name:      "DependencyEdge",
			value:     model.DependencyEdge{FromID: "a", ToID: "b"},
			forbidden: []string{"kind"},
		},
		{
			name:      "Digest",
			value:     model.Digest{Algorithm: model.DigestAlgorithmSHA256, Value: "abc123"},
			forbidden: []string{"subject"},
		},
		{
			name:      "Contact",
			value:     model.Contact{Kind: model.ContactKindOrganization, Name: "Acme Inc"},
			forbidden: []string{"url"},
		},
		{
			name:      "ExternalReference",
			value:     model.ExternalReference{Type: "website", Locator: "https://example.test"},
			forbidden: []string{"category", "comment", "hashes"},
		},
		{
			name:      "DependencyOrigin",
			value:     model.DependencyOrigin{ArtifactURL: "https://cdn.test/pkg.tgz"},
