...
func TestPluginRequestResponseRegistryJSON(t *testing.T) {
	...
	registry.Ensure("pkg:npm/lodash@4.17.15").Vulnerabilities = []model.Vulnerability{{ID: "GHSA-lodash", Source: "osv"}}
	...
	before := mustDependency(t, model.Coordinates{Name: "before"})
	before.Source = model.DependencySourceRegistry
	after := mustDependency(t, model.Coordinates{Name: "after"})
	after.Source = model.DependencySourceGit
	...
	if len(auditReq.DependencyDetailChanges) != 1 ||
		auditReq.DependencyDetailChanges[0].After.Source != model.DependencySourceGit {
...
func TestPackageUpdatesAreGatedOnTheWire(t *testing.T) {
	result := MatchResult{PackageUpdates: []*model.Package{{
		Coordinates: model.Coordinates{PURL: "pkg:npm/a@1.0.0"},
		Homepage:    "https://user:pw@evil.test/",
		Description: "bad\x07text",
		Supplier:    &model.Contact{Kind: model.ContactKindOrganization},
		Digests: []model.Digest{
			{Algorithm: "crc32", Value: "zz"},
			{Algorithm: model.DigestAlgorithmSHA256, Value: "ok"},
		},
	}}}
	...
	held := &model.Package{Coordinates: model.Coordinates{PURL: "pkg:npm/a@1.0.0"}, Homepage: "https://user:pw@evil.test/"}
