...
func TestRegistryPackagesForGraphSkipsFirstPartyNodes(t *testing.T) {
	...
	app := testkit.MustModuleNode(t, "pom.xml", model.Coordinates{
		Ecosystem: model.EcosystemMaven, PackageManager: model.PackageManagerMaven,
		Org: "com.acme", Name: "my-module", Version: "1.0.0",
		Type: model.PackageTypeApplication,
	})
	manifest, err := model.NewManifestNode("pom.xml", model.ManifestKindPomXML)
	if err != nil {
		t.Fatalf("NewManifestNode: %v", err)
	}
	pkg := testkit.MustDependencyNode(t, "pkg:maven/com.guava/guava@31.0")
	for _, node := range []model.GraphNode{app, manifest, pkg} {
...
func TestRegistryPackagesForGraphKeepsImportedApplicationComponents(t *testing.T) {
	graph := model.New()
	imported := testkit.MustDependencyCoords(t, model.Coordinates{
		Ecosystem: model.EcosystemNPM, Name: "bundled-app", Version: "2.0.0",
		Type: model.PackageTypeApplication,
	})
...
// The target parameter is typed *model.DependencyNode now, so a first-party
...
func TestRegistryPackagesForGraphTargetLimitsToTarget(t *testing.T) {
	...
	for _, node := range []model.GraphNode{target, other} {
		if err := graph.AddNode(node); err != nil {
			t.Fatalf("add node: %v", err)
		}
	}
...
func TestMissingLicensePackagesAndNormalizeLicenseSet(t *testing.T) {
	packages := []*model.Package{
		nil,
		{Coordinates: model.Coordinates{Name: "has-license", Version: "1.0.0"},
			Licenses: []model.PackageLicense{{Value: "MIT"}}},
		{Coordinates: model.Coordinates{Name: "", Version: "1.0.0"}},
		{Coordinates: model.Coordinates{Name: "no-version", Version: " "}},
		{Coordinates: model.Coordinates{Name: "eligible", Version: "2.0.0"}},
	}
	...
	if licenses[0].Value != "MIT" || licenses[0].SPDXExpression != "MIT" || licenses[0].Type != model.LicenseType("declared") {
